...
export default function Communities() {
  ...
  useEffect(() => {
    setCurrentPageTitle("Communities");
  ...
  // Filter for communities and management companies
  const communities = allTenants?.filter(t => t.type === 'community') || [];
  ...
  const saveMutation = useMutation({
    mutationFn: (data: typeof formData) =>
      isEditing && selectedCommunity
        ? api.updateTenant(selectedCommunity.id, { ...data, type: 'community' })
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `Community has been successfully ${isEditing ? 'updated' : 'created'}.`,
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Community has been successfully deactivated.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const filteredCommunities = communities.filter((community) => {
    ...
    const parentCompany = managementCompanies.find(mc => mc.id === community.managementCompanyId);
    return (
      community.name.toLowerCase().includes(searchLower) ||
      community.subdomain.toLowerCase().includes(searchLower) ||
  ...
  const handleEdit = (community: any) => {
    ...
    setFormData({
      ...
      managementCompanyId: community.managementCompanyId || "none",
    });
    setDialogOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all communities (HOAs/POAs) in the system
          </p>
        </div>
                  ...
                  <TableRow key={community.id}>
                    ...
                    <TableCell className="text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        {getManagementCompanyName(community.managementCompanyId)}
                      </div>
                    </TableCell>
                    <TableCell>
                      {community.demoCodeId ? (
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleEdit(community)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{isEditing ? "Edit" : "Create"} Community</DialogTitle>
            <DialogDescription>
              {isEditing ? "Update the community details." : "Add a new community to the system."}
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{selectedCommunity?.name}"? This will deactivate the community.
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteMutation.mutate(selectedCommunity?.id)}
